/**
 * Newmind Flow API Client
 * 
 * This module provides a client for interacting with the Newmind Flow API.
 */
...
export class NewflowApiClient {
  ...
  /**
   * Create a new Newmind Flow API client
   * 
  ...
  async checkConnectivity(): Promise<void> {
    try {
      ...
      throw handleAxiosError(error, 'Failed to connect to Newmind Flow API');
    }
  }
  ...
  async updateWorkflow(id: string, workflow: Record<string, any>): Promise<any> {
    try {
      // Remove read-only properties that cause issues with Newmind Flow API v1
      // According to Newmind Flow API schema, only name, nodes, connections, settings, and staticData are allowed
...
/**
 * Create and return a configured Newmind Flow API client
 * 
 * @param config Environment configuration
