# Newmind Flow MCP Server

[![npm version](https://badge.fury.io/js/%40TocharianOU%2Fnewflow-mcp-server.svg)](https://badge.fury.io/js/%40TocharianOU%2Fnewflow-mcp-server)

A Model Context Protocol (MCP) server that allows AI assistants to interact with Newmind Flow workflows through natural language.

## Overview

This project provides a Model Context Protocol (MCP) server that empowers AI assistants to seamlessly interact with Newmind Flow, a powerful workflow automation platform. It acts as a bridge, enabling AI assistants to programmatically manage and control Newmind Flow workflows and executions using natural language commands.

**Features:**
- ✅ **Dual Transport Support**: Stdio (default) and Streamable HTTP modes
- ✅ **Remote Access**: HTTP mode enables API integration and remote connections
- ✅ **Session Management**: Built-in session tracking for HTTP clients
...
- Node.js 20 or later
- Newmind Flow instance with API access enabled

### Install from npm

```bash
npm install -g @tocharianou/newflow-mcp-server
```
...
git clone https://github.com/TocharianOU/newflow-mcp-server.git
cd newflow-mcp-server
...
If you installed the server using `npm install -g @tocharianou/newflow-mcp-server`:

1.  Open your terminal or command prompt.
2.  Run the following command to get the latest version:
    ```bash
    npm install -g @tocharianou/newflow-mcp-server@latest
    ```
...
8.  Restart the server.
    *   If you run the server directly using a command like `node build/index.js` in your AI assistant's MCP configuration, ensure the path is still correct. Using `npm install -g .` and then `newflow-mcp-server` as the command should keep this consistent.
...
1.  Pull the latest image from Docker Hub:
    ```bash
    docker pull TocharianOU/newflow-mcp-server:latest
    ```
...
For local usage with Claude Desktop and other MCP clients:
...
For remote access, API integration, and web applications:
...
```json
{
  "mcpServers": {
    ...
    "newflow-local": {
      ...
      "args": [
        "/path/to/your/cloned/newflow-mcp-server/build/index.js"
        // On Windows, use double backslashes:
        // "C:\\path\\to\\your\\cloned\\newflow-mcp-server\\build\\index.js"
      ],
      ...
      "env": {
        ...
        // "NEWFLOW_WEBHOOK_PASSWORD": "your_webhook_password"
      },
...
*   Replace `/path/to/your/cloned/newflow-mcp-server/` with the actual absolute path where you cloned and built the repository.
*   Use the correct path separator for your operating system (forward slashes `/` for macOS/Linux, double backslashes `\\` for Windows).
*   Ensure you provide the correct `NEWFLOW_API_URL` (including `/api/v1`) and `NEWFLOW_API_KEY`.
...
The webhook authentication is handled automatically using the `NEWFLOW_WEBHOOK_USERNAME` and `NEWFLOW_WEBHOOK_PASSWORD` environment variables.
...
The Newmind Flow MCP Server is a community-driven project, and its future direction will be shaped by your feedback and contributions!
...
Please share your thoughts, feature requests, and ideas by opening an issue on our [GitHub Issues page](https://github.com/TocharianOU/newflow-mcp-server/issues). Let's build a powerful tool for AI assistants together!
...
We welcome contributions from the community and are excited to see how you can help improve the Newmind Flow MCP Server! Whether you're fixing a bug, proposing a new feature, or improving documentation, your help is valued.

### Reporting Bugs

If you encounter a bug, please report it by opening an issue on our [GitHub Issues page](https://github.com/TocharianOU/newflow-mcp-server/issues).

When submitting a bug report, please include the following:

-   A clear and descriptive title.
-   A detailed description of the problem, including steps to reproduce the bug.
-   Information about your environment (e.g., Node.js version, Newmind Flow MCP Server version, operating system).
-   Any relevant error messages or screenshots.

### Suggesting Enhancements

We're always looking for ways to make the server better. If you have an idea for an enhancement or a new feature, please open an issue on our [GitHub Issues page](https://github.com/TocharianOU/newflow-mcp-server/issues).
...
1.  **Fork the repository:** Create your own fork of the [newflow-mcp-server repository](https://github.com/TocharianOU/newflow-mcp-server).
...
6.  **Open a Pull Request (PR):** Submit a PR to the `main` branch of the official `newflow-mcp-server` repository.
...
1.  Check out our [GitHub Issues page](https://github.com/TocharianOU/newflow-mcp-server/issues) to find existing tasks, suggest new ideas, or express your interest in becoming a co-maintainer.
...
[![Star History Chart](https://api.star-history.com/svg?repos=TocharianOU/newflow-mcp-server&type=Date)](https://www.star-history.com/#TocharianOU/newflow-mcp-server&Date)
