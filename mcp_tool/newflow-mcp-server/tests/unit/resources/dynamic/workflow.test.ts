...
function getWorkflowResourceTemplateUri() {
  return 'newmindflow://workflows/{id}';
}

function extractWorkflowIdFromUri(uri: string): string | null {
  const regex = /^newmindflow:\/\/workflows\/([^/]+)$/;
...
describe('Workflow Resource URI Functions', () => {
  describe('getWorkflowResourceTemplateUri', () => {
    it('should return the correct URI template', () => {
      expect(getWorkflowResourceTemplateUri()).toBe('newmindflow://workflows/{id}');
    });
  });
  
  describe('extractWorkflowIdFromUri', () => {
    it('should extract workflow ID from valid URI', () => {
      expect(extractWorkflowIdFromUri('newmindflow://workflows/123abc')).toBe('123abc');
      expect(extractWorkflowIdFromUri('newmindflow://workflows/workflow-name-with-dashes')).toBe('workflow-name-with-dashes');
    });
    
    it('should return null for invalid URI formats', () => {
      expect(extractWorkflowIdFromUri('newmindflow://workflows/')).toBeNull();
      expect(extractWorkflowIdFromUri('newmindflow://workflows')).toBeNull();
      expect(extractWorkflowIdFromUri('newmindflow://workflow/123')).toBeNull();
