...
This page demonstrates more complex examples of using the newmindflow MCP Server with AI assistants.
...
const stats = await accessMcpResource('newmindflow-mcp-server', 'newmindflow://execution-stats');
...
for (const workflow of workflows) {
  ...
  const details = await useMcpTool('newmindflow-mcp-server', 'workflow_get', {
    id: workflow.id
  });
  
  // Get recent executions
  const executions = await useMcpTool('newmindflow-mcp-server', 'execution_list', {
  ...
  if (nodeTypes.has("newmindflow-nodes-base.httpRequest") && !nodeTypes.has("newmindflow-nodes-base.errorTrigger")) {
...
for (const workflow of workflows) {
  ...
  const scheduleNodes = details.nodes.filter(node => 
    node.type === "newmindflow-nodes-base.scheduleTrigger" || 
    node.type === "newmindflow-nodes-base.cron"
  );
...
for (const { workflow, scheduleNodes } of scheduledWorkflows) {
  ...
  for (const scheduleNode of scheduleNodes) {
    ...
    if (updatedNode.type === "newmindflow-nodes-base.scheduleTrigger") {
    ...
    } else if (updatedNode.type === "newmindflow-nodes-base.cron") {
  ...
  const result = await useMcpTool('newmindflow-mcp-server', 'workflow_update', {
    id: workflow.id,
...
const monitorWorkflow = await useMcpTool('newmindflow-mcp-server', 'workflow_create', {
  ...
  nodes: [
    ...
    {
      name: "Filter Empty",
      type: "newmindflow-nodes-base.filter",
      position: [500, 300],
      parameters: {
        conditions: {
          boolean: [
...
notificationText += "Check your newmindflow dashboard for more details.";

return [{
  json: {
    text: notificationText,
    subject: \`newmindflow Alert: \${Object.keys(workflowErrors).length} Workflow(s) Failed\`,
...
`
    ...
    {
      name: "Send Email",
      type: "newmindflow-nodes-base.emailSend",
      position: [900, 300],
      parameters: {
...
2. Set up the email configuration in newmindflow settings
...
You can customize the notification format, schedule, and notification method by editing the workflow in the newmindflow interface.
`;
```
...
const migrationWorkflow = await useMcpTool('newmindflow-mcp-server', 'workflow_create', {
  ...
  nodes: [
    ...
    },
    {
      ...
      type: "newmindflow-nodes-base.executeWorkflow",
      position: [300, 300],
      parameters: {
...
## Next Steps:
1. Open the workflow in the newmindflow interface
...
These examples demonstrate more advanced usage patterns for the newmindflow MCP Server. For integration with other systems, see the [Integration Examples](./integration-examples.md) page.
