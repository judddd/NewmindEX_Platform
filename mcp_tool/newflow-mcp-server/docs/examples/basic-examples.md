...
This page provides basic examples of using the newmindflow MCP Server with AI assistants.

## Listing Workflows

### User Prompt

"Show me all the workflows in my newmindflow instance."

### Assistant Actions

```javascript
// The assistant uses the workflow_list tool
const result = await useMcpTool('newmindflow-mcp-server', 'workflow_list', {});

// The assistant formats and presents the results
if (result.length === 0) {
  return "You don't have any workflows in your newmindflow instance yet.";
} else {
...
const execution = await useMcpTool('newmindflow-mcp-server', 'execution_get', {
...
const result = await useMcpTool('newmindflow-mcp-server', 'workflow_deactivate', {
...
const newWorkflow = await useMcpTool('newmindflow-mcp-server', 'workflow_create', {
...
You can view and edit this workflow in the newmindflow interface (ID: ${newWorkflow.id}), and then ask me to activate it when you're ready.
`;
```

These examples demonstrate the basic operations you can perform with the newmindflow MCP Server. For more complex scenarios, see the [Advanced Scenarios](./advanced-scenarios.md) page.
