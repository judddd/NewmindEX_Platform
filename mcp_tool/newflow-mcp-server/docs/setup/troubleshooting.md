...
This guide addresses common issues you might encounter when setting up and using the newmindflow MCP Server.

## Connection Issues

### Cannot Connect to newmindflow API

**Symptoms:** 
- Error messages mentioning "Connection refused" or "Cannot connect to newmindflow API"
- Timeout errors when trying to use MCP tools

**Possible Solutions:**
1. **Verify newmindflow is running:**
   - Ensure your newmindflow instance is running and accessible
   - Try accessing the newmindflow URL in a browser

2. **Check newmindflow API URL:**
...
3. **Network Configuration:**
   ...
   - Check if newmindflow is configured to allow remote connections
...
**Possible Solutions:**
1. **Verify API Key:**
   - Check that the `N8N_API_KEY` in your `.env` file matches the one in newmindflow
...
3. **newmindflow API Settings:**
   - Verify that API access is enabled in newmindflow settings
...
2. **newmindflow Version:**
   - Check if your newmindflow version supports all the API endpoints being used
   - Update newmindflow to the latest version if possible
...
2. **Workflow Status:**
   ...
   - Verify the workflow executes correctly in the newmindflow interface
...
1. **Check GitHub Issues:**
   - Look for similar issues in the [GitHub repository](https://github.com/yourusername/newmindflow-mcp-server/issues)
...
3. **Community Support:**
   - Ask in the newmindflow community forums
