...
This guide explains how to extend the newmindflow MCP Server with new functionality.

## Overview

The newmindflow MCP Server is designed to be extensible, allowing developers to add new tools and resources without modifying existing code. This extensibility makes it easy to support new newmindflow features or customize the server for specific use cases.
...
import { N8nClient } from '../../api/newmindflow-client.js';
...
export async function handleMyTool(
  ...
  try {
    ...
    // Use the N8nClient to interact with newmindflow
...
export const MY_RESOURCE_URI = 'newmindflow://my-resource';
...
export const MY_RESOURCE_URI_TEMPLATE = 'newmindflow://my-resource/{id}';

export function matchMyResourceUri(uri: string): { id: string } | null {
  const match = uri.match(/^newmindflow:\/\/my-resource\/([^/]+)$/);
...
If you need to add support for new newmindflow API features, extend the N8nClient class:

### 1. Add New Methods to the Client

```typescript
// src/api/newmindflow-client.ts
...
```typescript
// tests/unit/api/newmindflow-client.test.ts
describe('N8nClient', () => {
  // ... existing tests
...
## Example: Adding Support for newmindflow Tags

Here's a complete example of adding support for newmindflow tags:
...
export function getTagListToolDefinition(): ToolDefinition {
  return {
    name: 'tag_list',
    description: 'List all tags in newmindflow',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
...
export const TAGS_URI = 'newmindflow://tags';
...
By following these patterns, you can extend the newmindflow MCP Server to support any newmindflow feature or add custom functionality tailored to your specific needs.
