...
This document describes the architectural design of the newmindflow MCP Server.

## Overview

The newmindflow MCP Server follows a layered architecture pattern that separates concerns and promotes maintainability. The main architectural layers are:

1. **Transport Layer**: Handles communication with AI assistants via the Model Context Protocol
2. **API Client Layer**: Interacts with the newmindflow API
...
The API client layer (`src/api/`) provides a clean interface for interacting with the newmindflow API. It includes:

- `N8nClient`: The main client that encapsulates communication with newmindflow
- API-specific functionality divided by resource type (workflows, executions)
- Authentication handling using the newmindflow API key

The client uses Axios for HTTP requests and includes error handling specific to the newmindflow API responses.
...
- Error translation functions to convert newmindflow API errors to MCP errors
...
4. Tool/resource uses the newmindflow API client to interact with newmindflow
...
The newmindflow API client adapts the newmindflow API to the internal representation used by the server.
...
| `src/api/newmindflow-client.ts` | Main client for interacting with the newmindflow API |
...
3. **Supporting new newmindflow API features**: Extend the API client in `src/api/` to support new API endpoints or features
