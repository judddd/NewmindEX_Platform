...
This document describes the testing approach for the newmindflow MCP Server and provides guidelines for writing effective tests.

## Overview

The newmindflow MCP Server uses Jest as its testing framework and follows a multi-level testing approach:
...
- `src/api/newmindflow-client.ts` has a corresponding test at `tests/unit/api/newmindflow-client.test.ts`
...
import { N8nClient } from '../../../../src/api/newmindflow-client.js';
...
### newmindflow API Fixtures

Common fixtures for newmindflow API responses are stored in a shared file:

```typescript
// tests/mocks/newmindflow-fixtures.ts
...
Thorough testing is essential for maintaining a reliable and robust newmindflow MCP Server. By following these guidelines and examples, you can write effective tests that help ensure your code works as expected and catches issues early.
