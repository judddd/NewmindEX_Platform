...
This section provides a comprehensive reference for the newmindflow MCP Server API, including all available tools and resources.

## Overview

The newmindflow MCP Server implements the Model Context Protocol (MCP) to provide AI assistants with access to newmindflow workflows and executions. The API is divided into two main categories:

1. **Tools**: Executable functions that can perform operations on newmindflow, such as creating workflows or starting executions.
2. **Resources**: Data sources that provide information about workflows and executions.

## API Architecture

The newmindflow MCP Server follows a clean separation of concerns:

- **Client Layer**: Handles communication with the newmindflow API
...
All API interactions are authenticated using the newmindflow API key configured in your environment.
