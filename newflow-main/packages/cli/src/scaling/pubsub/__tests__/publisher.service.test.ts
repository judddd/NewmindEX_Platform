...
import { mockLogger } from '@newflow/backend-test-utils';
...
describe('Publisher', () => {
	...
	describe('publishCommand', () => {
		...
		it('should publish command into `newflow.commands` pubsub channel', async () => {
			const publisher = new Publisher(logger, redisClientService, instanceSettings);
			const msg = mock<PubSub.Command>({ command: 'reload-license' });
	...
	describe('publishWorkerResponse', () => {
		it('should publish worker response into `newflow.worker-response` pubsub channel', async () => {
			const publisher = new Publisher(logger, redisClientService, instanceSettings);
			...
			expect(client.publish).toHaveBeenCalledWith('newflow.worker-response', JSON.stringify(msg));
		});
	});
});
