...
export class TaskRunnerFailedHeartbeatError extends UserError {
	...
	constructor(heartbeatInterval: number, isSelfHosted: boolean) {
		...
		const fixes = {
			...
			increaseInterval: `If your task can reasonably keep the task runner busy for more than ${heartbeatInterval} ${heartbeatInterval === 1 ? 'second' : 'seconds'}, increase the heartbeat interval using the NEWFLOW_RUNNERS_HEARTBEAT_INTERVAL environment variable.`,
		};
