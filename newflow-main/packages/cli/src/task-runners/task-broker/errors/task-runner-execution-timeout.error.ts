...
import type { TaskRunnerMode } from '@newflow/config';
...
export class TaskRunnerExecutionTimeoutError extends OperationalError {
	...
	constructor({
		...
		const fixes = {
			...
			increaseTimeout: `If your task can reasonably take more than ${taskTimeout} ${taskTimeout === 1 ? 'second' : 'seconds'}, increase the timeout using the NEWFLOW_RUNNERS_TASK_TIMEOUT environment variable.`,
		};
