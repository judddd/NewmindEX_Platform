...
describe('TaskRunnerProcess', () => {
	...
	describe('start', () => {
		...
		it('should pass NEWFLOW_RUNNERS_TASK_TIMEOUT if set', async () => {
			jest.spyOn(authService, 'createGrantToken').mockResolvedValue('grantToken');
			...
			expect(options.env).toEqual(
				expect.objectContaining({
					NEWFLOW_RUNNERS_TASK_TIMEOUT: '123',
				}),
			);
		});

		it('should pass NEWFLOW_RUNNERS_HEARTBEAT_INTERVAL if set', async () => {
			jest.spyOn(authService, 'createGrantToken').mockResolvedValue('grantToken');
			...
			expect(options.env).toEqual(
				expect.objectContaining({
					NEWFLOW_RUNNERS_HEARTBEAT_INTERVAL: '456',
				}),
			);
		});
		...
		it('on secure mode, should use --disallow-code-generation-from-strings and --disable-proto=delete flags', async () => {
			...
			expect(spawnMock.mock.calls[0].at(1)).toEqual([
				...
				expect.stringContaining('/packages/@newflow/task-runner/dist/start.js'),
			]);
		});
