...
import { type InsightsSummary, type InsightsDateRange } from '@newflow/api-types';
import { LicenseState, Logger } from '@newflow/backend-common';
import { OnLeaderStepdown, OnLeaderTakeover } from '@newflow/decorators';
...
export class InsightsService {
	...
	settings() {
		// License restrictions removed - all features available
		return {
			summary: true,
			dashboard: true,
			dateRanges: this.getAllDateRanges(),
		};
	}

	/**
	 * Returns all available date ranges without license restrictions
	 */
	private getAllDateRanges(): DateRange[] {
		return INSIGHTS_DATE_RANGE_KEYS.map((key) => ({
			key,
			licensed: true, // All ranges are now available
	...
	getMaxAgeInDaysAndGranularity(
		...
		// License restrictions removed - all date ranges available
		const dateRange = this.getAllDateRanges().find((range) => range.key === dateRangeKey);
		...
		// License check removed - all ranges are now available
		return { ...dateRange, maxAgeInDays: keyRangeToDays[dateRangeKey] };
	}

	/**
	 * @deprecated Use getAllDateRanges() instead. This method kept for backward compatibility.
	 * Returns all available date ranges without license restrictions.
	 */
	getAvailableDateRanges(): DateRange[] {
		return this.getAllDateRanges();
	}
}
