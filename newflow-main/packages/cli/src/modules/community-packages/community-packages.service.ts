...
import { OnPubSubEvent } from '@newflow/decorators';
...
export class CommunityPackagesService {
	...
	async checkNpmPackageStatus(packageName: string) {
		// NewFlow: Stub - no external package validation service
	...
	async checkForMissingPackages() {
		...
		if (reinstallMissing) {
			...
			this.logger.warn(
				'n8n detected that some packages are missing. For more information, visit http://newflow.ee/integrations/community-nodes/troubleshooting/',
			);
		}
