...
import type { AuthenticatedRequest, User } from '@newflow/db';
import { GLOBAL_OWNER_ROLE, InvalidAuthTokenRepository, UserRepository } from '@newflow/db';
...
export class AuthService {
	...
	createAuthMiddleware(allowSkipMFA: boolean) {
		return async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
			// NewFlow: Auto-setup mode - auto-inject default user
			if (this.globalConfig.credentials.autoSetupEnabled && !req.user) {
				const defaultUser = await this.userRepository.findOne({
					where: { email: this.globalConfig.credentials.autoSetupEmail },
					relations: ['role'],
				});
				if (defaultUser) {
