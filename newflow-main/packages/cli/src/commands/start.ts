...
import { ExecutionRepository, SettingsRepository } from '@newflow/db';
...
import { EDITOR_UI_DIST_DIR, NEWFLOW_VERSION } from '@/constants';
...
const flagsSchema = z.object({
	...
	tunnel: z
		.boolean()
		.describe(
			'runs the webhooks via a hooks.newmindtech.cn tunnel server. Use only for testing and development!',
		)
		.optional(),
});
...
@Command({
	name: 'start',
	description: 'Starts NewFlow. Makes Web-UI available and starts active workflows',
...
export class Start extends BaseCommand<z.infer<typeof flagsSchema>> {
	...
	/**
	 * Stop NewFlow in a graceful way.
	...
	async stopProcess() {
		...
		try {
			...
			// NewFlow: Multi-main removed - always single instance
			...
			await this.exitWithCrash('There was an error shutting down NewFlow.', error);
		}
	...
	private generateConfigTags() {
		const frontendSentryConfig = JSON.stringify({
			...
			release: `NewFlow@${NEWFLOW_VERSION}`,
		});
		...
		const configMetaTags = [
			`<meta name="newflow:config:rest-endpoint" content="${restEndpointEncoded}">`,
			`<meta name="newflow:config:sentry" content="${sentryConfigEncoded}">`,
		].join('');
	...
	async init() {
		...
		this.logger.info('Initializing NewFlow process');
		if (config.getEnv('executions.mode') === 'queue') {
	...
	async initOrchestration() {
		...
		await subscriber.subscribe('newflow.commands');
		await subscriber.subscribe('newflow.worker-response');

		// NewFlow: Multi-main removed - always mark as leader
		// if (this.instanceSettings.isMultiMain) {
	...
	async run() {
		...
		if (flags.tunnel) {
			...
			let tunnelSubdomain =
				process.env.NEWFLOW_TUNNEL_SUBDOMAIN ?? this.instanceSettings.tunnelSubdomain ?? '';
			...
			const webhookTunnel = await localtunnel(port, {
				host: 'https://hooks.newmindtech.cn',
			...
			this.log(
				'IMPORTANT! Do not share with anybody as it would give people access to your NewFlow instance!',
			);
		}
		...
		// Allow to open NewFlow editor by pressing "o"
