...
export class Worker extends BaseCommand<z.infer<typeof flagsSchema>> {
	 ...
	 *
	 * Taken from env var `NEWFLOW_CONCURRENCY_PRODUCTION_LIMIT` if set to a value
	...
	async init() {
		...
		if (QUEUE_WORKER_TIMEOUT) {
			...
			this.logger.warn(
				'QUEUE_WORKER_TIMEOUT has been deprecated. Rename it to NEWFLOW_GRACEFUL_SHUTDOWN_TIMEOUT.',
			);
		}
	...
	async initOrchestration() {
		...
		await Container.get(Subscriber).subscribe('newflow.commands');
		// NewFlow: Multi-main removed
	...
	async run() {
		...
		this.logger.info(` * Version: ${NEWFLOW_VERSION}`);
