...
import { NEWFLOW_VERSION, NEWFLOW_RELEASE_DATE } from '@/constants';
import * as CrashJournal from '@/crash-journal';
import { getDataDeduplicationService } from '@/deduplication';
import { DeprecationService } from '@/deprecation/deprecation.service';
// Test runner enterprise feature has been removed
...
export abstract class BaseCommand<F = never> {
	...
	async init(): Promise<void> {
		...
		await this.errorReporter.init({
			...
			dsn: backendDsn,
			environment,
			release: `n8n@${NEWFLOW_VERSION}`,
			serverName: deploymentName,
			releaseDate: NEWFLOW_RELEASE_DATE,
		...
		// NewFlow: Auto-setup for appliance/offline deployments
		if (this.globalConfig.credentials.autoSetupEnabled) {
			const { AutoSetupService } = await import('@/services/auto-setup.service');
			await Container.get(AutoSetupService).ensureAutoSetup();
		}
	...
	async initObjectStoreService() {
		...
		if (isSelected && !isAvailable) {
			throw new UserError(
				'External storage selected but unavailable. Please make external storage available by adding "s3" to `NEWFLOW_AVAILABLE_BINARY_DATA_MODES`.',
			);
		}
		...
		if (!isLicensed) {
			this.logger.error(
				'No license found for S3 storage. \n Either set `NEWFLOW_DEFAULT_BINARY_DATA_MODE` to something else, or upgrade to a license that supports this feature.',
			);
