...
import type { Logger, LicenseState, ModuleRegistry } from '@newflow/backend-common';
import type { GlobalConfig, SecurityConfig } from '@newflow/config';
...
describe('FrontendService', () => {
	...
	describe('envFeatureFlags functionality', () => {
		...
		describe('collectEnvFeatureFlags', () => {
			it('should collect environment variables with NEWFLOW_ENV_FEAT_ prefix', () => {
				process.env = {
					NEWFLOW_ENV_FEAT_TEST_FLAG: 'true',
					NEWFLOW_ENV_FEAT_ANOTHER_FLAG: 'false',
					NEWFLOW_ENV_FEAT_NUMERIC_FLAG: '123',
			...
			it('should return empty object when no NEWFLOW_ENV_FEAT_ variables are set', () => {
				process.env = {
			...
			it('should filter out undefined environment variable values', () => {
				process.env = {
					NEWFLOW_ENV_FEAT_DEFINED_FLAG: 'true',
					NEWFLOW_ENV_FEAT_UNDEFINED_FLAG: undefined,
				};
				...
				expect(result).toEqual({
					NEWFLOW_ENV_FEAT_DEFINED_FLAG: 'true',
					// NEWFLOW_ENV_FEAT_UNDEFINED_FLAG should be filtered out
				});
			});
		});
		...
		describe('settings integration', () => {
			it('should include envFeatureFlags in initial settings', () => {
				process.env = {
					NEWFLOW_ENV_FEAT_INIT_FLAG: 'true',
					NEWFLOW_ENV_FEAT_ANOTHER_FLAG: 'false',
				};
			...
			it('should refresh envFeatureFlags when getSettings is called', () => {
				process.env = {
					NEWFLOW_ENV_FEAT_INITIAL_FLAG: 'true',
				};
				...
				// Change environment
				process.env = {
					NEWFLOW_ENV_FEAT_INITIAL_FLAG: 'false',
					NEWFLOW_ENV_FEAT_NEW_FLAG: 'true',
				};
