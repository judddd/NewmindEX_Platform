...
import type { FrontendSettings, ITelemetrySettings, N8nEnvFeatFlags } from '@newflow/api-types';
import { LicenseState, Logger, ModuleRegistry } from '@newflow/backend-common';
import { GlobalConfig, SecurityConfig } from '@newflow/config';
...
import { inE2ETests, NEWFLOW_VERSION } from '@/constants';
...
import { getCurrentAuthenticationMethod } from '@/auth/sso-helpers';
...
export class FrontendService {
	...
	private collectEnvFeatureFlags(): N8nEnvFeatFlags {
		...
		for (const [key, value] of Object.entries(process.env)) {
			if (key.startsWith('NEWFLOW_ENV_FEAT_') && value !== undefined) {
	...
	private initSettings() {
		...
		this.settings = {
			...
			previewMode: process.env.NEWFLOW_PREVIEW_MODE === 'true',
			...
			userManagement: {
				...
				showSetupOnFirstLoad: this.globalConfig.credentials.autoSetupEnabled
					? false
					: !config.getEnv('userManagement.isInstanceOwnerSetUp'),
				smtpSetup: this.mailer.isEmailSetUp,
				authenticationMethod: getCurrentAuthenticationMethod(),
			},
			// NewFlow: Auto-setup mode configuration
			autoSetup: {
				enabled: this.globalConfig.credentials.autoSetupEnabled,
			},
			// NewFlow: SSO features removed - only email authentication is supported
			sso: {
				saml: {
					loginEnabled: false,
					loginLabel: '',
			...
			aiAssistant: {
				enabled: true, // NewFlow: Using CustomAI (LangChain + OpenAI-compatible API)
			},
			templates: {
			...
			isMultiMain: false, // NewFlow: Multi-main removed, always single instance
			...
			// NewFlow: Simplified enterprise settings for offline mode
			enterprise: {
				sharing: false,
				ldap: false,
				saml: false,
				...
				showNonProdBanner: false, // NewFlow: Always hide non-production banner
				debugInEditor: true, // NewFlow: Enable debug in editor
				binaryDataS3: false,
				workflowHistory: true, // NewFlow: Self-controlled workflow history
				workerView: true, // NewFlow: Worker view enabled
				advancedPermissions: false,
				apiKeyScopes: false,
				workflowDiffs: false,
				projects: {
					team: {
						limit: 0, // NewFlow: Team projects disabled
					},
				},
			},
			mfa: {
			...
			// NewFlow: Folders disabled (depends on Projects feature)
			folders: {
				enabled: false,
			},
			evaluation: {
			...
			docsBaseUrl: this.globalConfig.docs.baseUrl,
		};
	}
	...
	getSettings(): FrontendSettings {
		...
		// OIDC enterprise feature has been removed

		// NewFlow: Variables feature has been removed
