...
import { inDevelopment, inProduction } from '@newflow/backend-common';
import { SecurityConfig } from '@newflow/config';
import { Time } from '@newflow/constants';
import type { APIRequest } from '@newflow/db';
...
export class Server extends AbstractServer {
	...
	async start() {
		...
		if (inDevelopment && process.env.NEWFLOW_DEV_RELOAD === 'true') {
	...
	private async registerAdditionalControllers() {
		...
		// SAML enterprise feature has been removed
		...
		// Source control enterprise feature has been removed

		// Variables enterprise feature has been removed
	}
	...
	async configure(): Promise<void> {
		...
		if (push.isBidirectional) {
			...
			this.logger.warn(
				'Collaboration features are disabled because push is configured unidirectional. Use NEWFLOW_PUSH_BACKEND=websocket environment variable to enable them.',
			);
		}
		...
		if (frontendService) {
			...
			const isPreviewMode = process.env.NEWFLOW_PREVIEW_MODE === 'true';
