...
export class DeprecationService {
	private readonly deprecations: Deprecation[] = [
		{ envVar: 'NEWFLOW_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'NEWFLOW_PERSISTED_BINARY_DATA_TTL', message: SAFE_TO_REMOVE },
		{ envVar: 'EXECUTIONS_DATA_PRUNE_TIMEOUT', message: SAFE_TO_REMOVE },
		{
			envVar: 'NEWFLOW_BINARY_DATA_MODE',
			message: '`default` is deprecated. Please switch to `filesystem` mode.',
			checkValue: (value: string) => value === 'default',
		},
		{
			envVar: 'NEWFLOW_CONFIG_FILES',
			message: 'Please use .env files or *_FILE env vars instead.',
		},
		{
			envVar: 'DB_TYPE',
		...
		},
		{
			...
			message:
				'Running SQLite without a pool of read connections is deprecated. Please set `DB_SQLITE_POOL_SIZE` to a value higher than zero. See: http://newflow.ee/hosting/configuration/environment-variables/database/#sqlite',
			checkValue: (_: string) => this.globalConfig.database.isLegacySqlite,
		},
		{
			envVar: 'NEWFLOW_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN',
			message: `n8n no longer deregisters webhooks at startup and shutdown. ${SAFE_TO_REMOVE}`,
		},
		{
			envVar: 'NEWFLOW_RUNNERS_ENABLED',
			message:
				'Running n8n without task runners is deprecated. Task runners will be turned on by default in a future version. Please set `NEWFLOW_RUNNERS_ENABLED=true` to enable task runners now and avoid potential issues in the future. Learn more: http://newflow.ee/hosting/configuration/task-runners/',
		...
		{
			envVar: 'NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT',
		...
		},
		{
			envVar: 'EXECUTIONS_PROCESS',
			message:
				'n8n does not support `own` mode since May 2023. Please remove this environment variable to allow n8n to start. If you need the isolation and performance gains, please consider queue mode: http://newflow.ee/hosting/scaling/queue-mode/',
			checkValue: (value: string) => value === 'own',
		},
		{
			envVar: 'NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE',
			message:
				'The default value of NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE will be changed from false to true in a future version. If you need to access environment variables from the Code Node or from expressions, please set NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE=false. Learn more: http://newflow.ee/hosting/configuration/environment-variables/security/',
			checkValue: (value: string | undefined) => value === undefined || value === '',
		},
		{
			envVar: 'NEWFLOW_GIT_NODE_DISABLE_BARE_REPOS',
			message:
				'Support for bare repositories in the Git Node will be removed in a future version due to security concerns. If you are not using bare repositories in the Git Node, please set NEWFLOW_GIT_NODE_DISABLE_BARE_REPOS=true. Learn more: http://newflow.ee/hosting/configuration/environment-variables/security/',
			checkValue: (value: string | undefined) => value === undefined || value === '',
		},
	];
