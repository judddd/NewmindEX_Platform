...
import type { InstanceType } from '@newflow/constants';
...
describe('DeprecationService', () => {
	...
	describe('NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT', () => {
		test('supports multiple warnings for the same environment variable', () => {
			// ARRANGE
			process.env.NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT = '1';
	...
	test.each([
		['NEWFLOW_BINARY_DATA_TTL', '1', true],
		['NEWFLOW_PERSISTED_BINARY_DATA_TTL', '1', true],
		['EXECUTIONS_DATA_PRUNE_TIMEOUT', '1', true],
		['NEWFLOW_CONFIG_FILES', '1', true],
		['NEWFLOW_SKIP_WEBHOOK_DEREGISTRATION_SHUTDOWN', '1', true],
		['NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT', '1', true],
		['NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT', '2', true],
		['NEWFLOW_PARTIAL_EXECUTION_VERSION_DEFAULT', undefined, false],
	...
	test.each([
		...
		toTest('NEWFLOW_BINARY_DATA_MODE', mode, mustWarn);
	});
	...
	describe('NEWFLOW_RUNNERS_ENABLED', () => {
		const envVar = 'NEWFLOW_RUNNERS_ENABLED';
	...
	describe('NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE', () => {
		beforeEach(() => {
			process.env = {
				NEWFLOW_RUNNERS_ENABLED: 'true',
				N8N_GIT_NODE_DISABLE_BARE_REPOS: 'false',
		...
		test('should warn when N8N_BLOCK_ENV_ACCESS_IN_NODE is not set', () => {
			delete process.env.NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE;
			deprecationService.warn();
			expect(logger.warn).toHaveBeenCalled();
		});
		...
		test.each(['false', 'true'])(
			...
			(value) => {
				process.env.NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE = value;
				deprecationService.warn();
				expect(logger.warn).not.toHaveBeenCalled();
			},
		);
	...
	describe('NEWFLOW_GIT_NODE_DISABLE_BARE_REPOS', () => {
		beforeEach(() => {
			process.env = {
				NEWFLOW_RUNNERS_ENABLED: 'true',
				N8N_BLOCK_ENV_ACCESS_IN_NODE: 'false',
		...
		test('should warn when N8N_GIT_NODE_DISABLE_BARE_REPOS is not set', () => {
			delete process.env.NEWFLOW_GIT_NODE_DISABLE_BARE_REPOS;
			deprecationService.warn();
			expect(logger.warn).toHaveBeenCalled();
		});
		...
		test.each(['false', 'true'])(
			...
			(value) => {
				process.env.NEWFLOW_GIT_NODE_DISABLE_BARE_REPOS = value;
				deprecationService.warn();
				expect(logger.warn).not.toHaveBeenCalled();
			},
		);
