...
import { OnShutdown } from '@newflow/decorators';
...
import { IExecutionResponse } from '@newflow/db';
 ...
 */
const NEWFLOW_CONTINUE = 'n8n|continue';
/**
 * send message for heartbeat check
 */
const NEWFLOW_HEARTBEAT = 'n8n|heartbeat';
/**
 * frontend did acknowledge the heartbeat
 */
const NEWFLOW_HEARTBEAT_ACK = 'n8n|heartbeat-ack';
...
export class ChatService {
	...
	private async processWaitingExecution(
		...
		if (lastNode && shouldResumeImmediately(lastNode)) {
			session.connection.send(NEWFLOW_CONTINUE);
	...
	private incomingMessageHandler(sessionKey: string) {
		return async (data: RawData) => {
			try {
				...
				if (message === NEWFLOW_HEARTBEAT_ACK) {
		...
		try {
			...
			for (const [key, session] of this.sessions.entries()) {
				...
				if (timeSinceLastHeartbeat > HEARTBEAT_TIMEOUT) {
					...
					try {
						session.connection.send(NEWFLOW_HEARTBEAT);
					} catch (e) {
						this.cleanupSession(session, key);
						const error = ensureError(e);
						this.errorReporter.error(error);
