...
export class CredentialsController {
	...
	async getOne(
		...
		// Enterprise credentials service has been removed - using base service
		const { shared, ...credential } = await this.credentialsService.getOne(
			req.user,
			credentialId,
			query.includeData,
		);
	...
	async shareCredentials(req: CredentialRequest.Share) {
		...
		await dbManager.transaction(async (trx) => {
			...
			// TODO: Implement basic credential sharing if needed
	...
	async transfer(req: CredentialRequest.Transfer) {
		...
		throw new BadRequestError('Credential transfer is not available in this version');
	}
}
