...
describe('shouldSkipAuthOnOAuthCallback', () => {
	const originalEnv = process.env.NEWFLOW_SKIP_AUTH_ON_OAUTH_CALLBACK;

	afterEach(() => {
		// Restore original environment variable after each test
		if (originalEnv === undefined) {
			delete process.env.NEWFLOW_SKIP_AUTH_ON_OAUTH_CALLBACK;
		} else {
			process.env.NEWFLOW_SKIP_AUTH_ON_OAUTH_CALLBACK = originalEnv;
		}
	});
	...
	describe('with various environment variable values', () => {
		...
		test.each(testCases)('"%s" value should return %s', ({ value, expected }) => {
			process.env.NEWFLOW_SKIP_AUTH_ON_OAUTH_CALLBACK = value;
