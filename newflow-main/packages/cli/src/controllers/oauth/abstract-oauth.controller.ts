...
import type { AuthenticatedRequest, CredentialsEntity, ICredentialsDb } from '@newflow/db';
import { CredentialsRepository } from '@newflow/db';
...
export function shouldSkipAuthOnOAuthCallback() {
	...
	const value = process.env.NEWFLOW_SKIP_AUTH_ON_OAUTH_CALLBACK?.toLowerCase() ?? 'true';
	return value === 'true';
}
