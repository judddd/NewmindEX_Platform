...
import { LICENSE_FEATURES, LICENSE_QUOTAS, UNLIMITED_LICENSE_QUOTA } from '@newflow/constants';
import {
	GLOBAL_ADMIN_ROLE,
	GLOBAL_MEMBER_ROLE,
	GLOBAL_OWNER_ROLE,
...
import { Get, Patch, Post, RestController } from '@newflow/decorators';
...
export class E2EController {
	...
	async setEnvFeatureFlags(req: Request<{}, {}, { flags: Record<string, string> }>) {
		...
		// Validate that all flags start with NEWFLOW_ENV_FEAT_
		for (const key of Object.keys(flags)) {
			if (!key.startsWith('NEWFLOW_ENV_FEAT_')) {
				return {
					success: false,
					message: `Invalid flag key: ${key}. Must start with NEWFLOW_ENV_FEAT_`,
				};
			}
		}

		// Clear existing NEWFLOW_ENV_FEAT_ environment variables
