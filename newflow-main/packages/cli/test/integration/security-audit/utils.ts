...
export const MOCK_09990_NEWFLOW_VERSION = {
	name: '0.999.0',
	nodes: [
		{
	...
	documentationUrl: 'http://newflow.ee/reference/release-notes/#n8n09990',
...
export const MOCK_01110_NEWFLOW_VERSION = {
	name: '0.111.0',
	nodes: [],
	createdAt: '2022-01-01T00:00:00.000Z',
	description:
		'Includes <strong>new nodes</strong>, <strong>node enhancements</strong>, <strong>core functionality</strong> and <strong>bug fixes</strong>',
	documentationUrl: 'http://newflow.ee/reference/release-notes/#n8n01110',
	hasBreakingChange: false,
	hasSecurityFix: false,
	hasSecurityIssue: false,
	securityIssueFixVersion: null,
...
export function simulateOutdatedInstanceOnce(versionName = MOCK_01110_NEWFLOW_VERSION.name) {
	const baseUrl = Container.get(GlobalConfig).versionNotifications.endpoint + '/';

	// @ts-expect-error readonly export
	constants.NEWFLOW_VERSION = versionName;

	nock(baseUrl)
		.get(versionName)
		.reply(200, [MOCK_01110_NEWFLOW_VERSION, MOCK_09990_NEWFLOW_VERSION]);
}

export function simulateUpToDateInstance(versionName = MOCK_09990_NEWFLOW_VERSION.name) {
	const baseUrl = Container.get(GlobalConfig).versionNotifications.endpoint + '/';

	// @ts-expect-error readonly export
	constants.NEWFLOW_VERSION = versionName;

	nock(baseUrl).persist().get(versionName).reply(200, [MOCK_09990_NEWFLOW_VERSION]);
}
