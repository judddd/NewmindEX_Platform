import { createWorkflow, testDb, mockInstance } from '@newflow/backend-test-utils';
import { GlobalConfig } from '@newflow/config';
import { WorkflowHistoryRepository } from '@newflow/db';
...
describe('Workflow History Manager', () => {
	...
	test('should not prune when disabled in config', async () => {
		globalConfig.workflowHistory.enabled = false;
		await createWorkflowHistory();
		await pruneAndAssertCount();
	});

	test('should not prune when prune time is -1 (infinite)', async () => {
		await createWorkflowHistory();
		await pruneAndAssertCount();
	});
