...
async function pullImagesForCI() {
	console.log(`🔄 Pulling images for ${process.env.NEWFLOW_DOCKER_IMAGE}...`);
	const stack = await createN8NStack({
		postgres: true,
	});

	console.log(`🔄 Images pulled for ${process.env.NEWFLOW_DOCKER_IMAGE}...`);
...
async function globalSetup() {
	...
	const n8nBaseUrl = process.env.NEWFLOW_BASE_URL;
	if (!n8nBaseUrl) {
		console.log('⚠️  N8N_BASE_URL environment variable is not set, skipping database reset');
		if (process.env.CI && process.env.NEWFLOW_DOCKER_IMAGE) {
