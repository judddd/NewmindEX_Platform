...
test.describe
	.serial('Environment Feature Flags', () => {
		test('should set feature flags at runtime and load it back in envFeatureFlags from backend settings', async ({
			...
			expect(setResponse.data.flags['NEWFLOW_ENV_FEAT_TEST']).toBe('true');

			const currentFlags = await api.getEnvFeatureFlags();

			expect(currentFlags).toBeInstanceOf(Object);
			expect(currentFlags.data['NEWFLOW_ENV_FEAT_TEST']).toBe('true');
		});
		...
		test('should reset feature flags at runtime', async ({ api }) => {
			...
			expect(setResponse1.data.flags['NEWFLOW_ENV_FEAT_TEST']).toBe('true');
			...
			expect(clearResponse.data.flags['NEWFLOW_ENV_FEAT_TEST']).toBeUndefined();

			const currentFlags = await api.getEnvFeatureFlags();

			expect(currentFlags).toBeInstanceOf(Object);
			expect(currentFlags.data['NEWFLOW_ENV_FEAT_TEST']).toBeUndefined();
		});
	});
