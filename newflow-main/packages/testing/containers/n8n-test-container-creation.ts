...
const BASE_ENV: Record<string, string> = {
	...
	N8N_LICENSE_TENANT_ID: process.env.NEWFLOW_LICENSE_TENANT_ID ?? '1001',
	N8N_LICENSE_ACTIVATION_KEY: process.env.NEWFLOW_LICENSE_ACTIVATION_KEY ?? '',
};

// Wait strategy for n8n main containers
const NEWFLOW_MAIN_WAIT_STRATEGY = Wait.forAll([
...
const NEWFLOW_WORKER_WAIT_STRATEGY = Wait.forAll([
	Wait.forListeningPorts(),
...
export async function createN8NStack(config: N8NConfig = {}): Promise<N8NStack> {
	...
	if (queueConfig) {
		...
		if (queueConfig.mains > 1) {
			if (!process.env.NEWFLOW_LICENSE_ACTIVATION_KEY) {
				throw new Error('NEWFLOW_LICENSE_ACTIVATION_KEY is required for multi-main instances');
			}
			environment = {
				...environment,
	...
	if (taskRunnerEnabled) {
		environment = {
			...
			NEWFLOW_RUNNERS_MODE: 'external',
			NEWFLOW_RUNNERS_AUTH_TOKEN: 'test',
			NEWFLOW_RUNNERS_BROKER_LISTEN_ADDRESS: '0.0.0.0',
		};
	}
...
async function createN8NContainer({
	...
	const taskRunnerEnabled = environment.NEWFLOW_RUNNERS_ENABLED === 'true';
