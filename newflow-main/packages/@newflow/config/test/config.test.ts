...
describe('GlobalConfig', () => {
	...
	describe('string unions', () => {
		it('on invalid value, should warn and fall back to default value', () => {
			process.env = {
				NEWFLOW_RUNNERS_MODE: 'non-existing-mode',
			...
			expect(consoleWarnMock).toHaveBeenCalledWith(
				expect.stringContaining(
					"Invalid value for NEWFLOW_RUNNERS_MODE - Invalid enum value. Expected 'internal' | 'external', received 'non-existing-mode'. Falling back to default value.",
				),
			);
