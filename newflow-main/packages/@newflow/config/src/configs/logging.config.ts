...
export class CronLoggingConfig {
	 ...
	 *
	 * @example `NEWFLOW_LOG_CRON_ACTIVE_INTERVAL=30` will log active crons every 30 minutes.
	 */
	@Env('NEWFLOW_LOG_CRON_ACTIVE_INTERVAL')
...
class FileLoggingConfig {
	 ...
	 *
	 * @example `NEWFLOW_LOG_FILE_COUNT_MAX=7` will keep at most 7 files.
	 * @example `NEWFLOW_LOG_FILE_COUNT_MAX=7d` will keep at most 7 days worth of files.
	 */
	@Env('NEWFLOW_LOG_FILE_COUNT_MAX')
	fileCountMax: number = 100;

	/** Max size (in MiB) for each log file. Only for `file` log output. */
	@Env('NEWFLOW_LOG_FILE_SIZE_MAX')
	fileSizeMax: number = 16;

	/** Location of the log files inside `~/.n8n`. Only for `file` log output. */
	@Env('NEWFLOW_LOG_FILE_LOCATION')
...
export class LoggingConfig {
	 ...
	 *
	 * @example `NEWFLOW_LOG_LEVEL=info` will output `error`, `warn` and `info` logs, but not `debug`.
	 */
	@Env('NEWFLOW_LOG_LEVEL', logLevelSchema)
	level: LogLevel = 'info';

	/**
	 * Where to output logs to. Options are: `console` or `file` or both in a comma separated list.
	 *
	 * @example `NEWFLOW_LOG_OUTPUT=console,file` will output to both console and file.
	 */
	@Env('NEWFLOW_LOG_OUTPUT')
	 ...
	 *
	 * @example
	 * `NEWFLOW_LOG_SCOPES=license`
	 * `NEWFLOW_LOG_SCOPES=license,waiting-executions`
	 */
	@Env('NEWFLOW_LOG_SCOPES')
