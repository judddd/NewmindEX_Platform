...
export class InstanceSettingsConfig {
	 ...
	 */
	@Env('NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS')
	...
	constructor() {
		...
		this.userHome = process.env.NEWFLOW_USER_FOLDER ?? process.env[homeVarName] ?? process.cwd();

		this.n8nFolder = path.join(this.userHome, '.newflow');
	}
}
