...
export { DocsConfig } from './configs/docs.config';
...
export class GlobalConfig {
	...
	/** Path NewFlow is deployed to */
	@Env('NEWFLOW_PATH')
	path: string = '/';

	/** Host name NewFlow can be reached */
	@Env('NEWFLOW_HOST')
	host: string = 'localhost';

	/** HTTP port NewFlow can be reached */
	@Env('NEWFLOW_PORT')
	port: number = 5678;

	/** IP address NewFlow should listen on */
	@Env('NEWFLOW_LISTEN_ADDRESS')
	listen_address: string = '::';

	/** HTTP Protocol via which NewFlow can be reached */
	@Env('NEWFLOW_PROTOCOL', protocolSchema)
