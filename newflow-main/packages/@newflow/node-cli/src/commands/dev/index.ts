...
export default class Dev extends Command {
	...
	static override flags = {
		'external-n8n': Flags.boolean({
			default: false,
			description:
				'By default n8n-node dev will run n8n in a sub process. Enable this option if you would like to run n8n elsewhere. Make sure to set NEWFLOW_DEV_RELOAD to true in that case.',
		}),
				...
				await Promise.race([
					new Promise<void>((resolve) => {
						runPersistentCommand('npx', ['-y', '--quiet', '--prefer-online', 'n8n@latest'], {
							...
							env: {
								...process.env,
								NEWFLOW_DEV_RELOAD: 'true',
								NEWFLOW_RUNNERS_ENABLED: 'true',
								DB_SQLITE_POOL_SIZE: '10',
								NEWFLOW_USER_FOLDER: n8nUserFolder,
							},
							name: 'n8n',
