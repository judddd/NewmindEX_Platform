...
export function getConfig(): MemoryVectorStoreConfig {
	...
	if (process.env.NEWFLOW_VECTOR_STORE_MAX_MEMORY) {
		const parsed = parseInt(process.env.NEWFLOW_VECTOR_STORE_MAX_MEMORY, 10);
		if (!isNaN(parsed)) {
			maxMemoryMB = parsed;
		}
	}
	...
	if (process.env.NEWFLOW_VECTOR_STORE_TTL_HOURS) {
		const parsed = parseInt(process.env.NEWFLOW_VECTOR_STORE_TTL_HOURS, 10);
		if (!isNaN(parsed)) {
			ttlHours = parsed;
		}
	}
