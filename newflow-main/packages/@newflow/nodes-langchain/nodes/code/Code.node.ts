...
export const vmResolver = makeResolverFromLegacyOptions({
	...
	resolve(moduleName, parentDirname) {
		if (moduleName.match(/^langchain\//) ?? moduleName.match(/^@langchain\//)) {
			return require.resolve(`@newflow/n8n-nodes-langchain/node_modules/${moduleName}.cjs`, {
				paths: [parentDirname],
			});
		}
		return;
		...
		codex: {
			...
			resources: {
				primaryDocumentation: [
					{
						url: 'http://newflow.ee/integrations/builtin/cluster-nodes/root-nodes/n8n-nodes-langchain.code/',
					},
				],
			},
		},
