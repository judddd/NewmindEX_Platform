...
import type { InsightsDateRange, InsightsSummaryType } from '@newflow/api-types';
import { useI18n } from '@newflow/i18n';
import { computed, defineAsyncComponent, onBeforeMount, onMounted, ref, watch } from 'vue';
import { useRoute } from 'vue-router';
import { INSIGHT_TYPES, TELEMETRY_TIME_RANGE } from '../insights.constants';
import InsightsDateRangeSelect from './InsightsDateRangeSelect.vue';

// Enterprise upgrade components removed - feature is now fully available
...
function handleTimeChange(value: InsightsDateRange['key']) {
	// Enterprise paywall removed - all time ranges now available
	...
	<div :class="$style.insightsView">
		<div :class="$style.insightsContainer">
			...
			<div :class="$style.insightsContent">
				<!-- Dashboard always enabled - license restrictions removed -->
				...
				<!-- InsightsPaywall removed - feature is now fully available -->
			</div>
		</div>
	</div>
</template>
