...
import type { FrontendSettings, N8nEnvFeatFlags, N8nEnvFeatFlagValue } from '@newflow/api-types';
...
describe('EnvFeatureFlag', () => {
	...
	beforeEach(() => {
		Object.keys(import.meta.env).forEach((key) => {
			if (key.startsWith('NEWFLOW_ENV_FEAT_')) {
	...
	test.each<[N8nEnvFeatFlagValue, Uppercase<string>, boolean]>([
		...
		(value, flagName, shouldRender) => {
			const envKey: keyof N8nEnvFeatFlags = `NEWFLOW_ENV_FEAT_${flagName}`;
	...
	it('should work with different flag names', () => {
		settingsStore.settings.envFeatureFlags = {
			NEWFLOW_ENV_FEAT_WORKFLOW_DIFFS: 'true',
			NEWFLOW_ENV_FEAT_ANOTHER_FEATURE: 'false',
		};
	...
	describe('runtime vs build-time priority', () => {
		it('should prioritize runtime settings over build-time env vars', () => {
			...
			(import.meta.env as N8nEnvFeatFlags).NEWFLOW_ENV_FEAT_TEST_FLAG = 'true';

			// Set runtime setting to override
			settingsStore.settings.envFeatureFlags = {
				NEWFLOW_ENV_FEAT_TEST_FLAG: 'false',
			};
