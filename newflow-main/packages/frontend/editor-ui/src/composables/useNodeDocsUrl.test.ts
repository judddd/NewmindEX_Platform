...
describe('useNodeDocsUrl', () => {
	...
	it('returns codex primaryDocumentation url with UTM params', () => {
		const nodeType = mock<INodeTypeDescription>({
			...
			codex: {
				resources: {
					primaryDocumentation: [{ url: 'http://newflow.ee/nodes/MyNode' }],
				},
			},
		});
		...
		expect(docsUrl.value).toEqual(
			'http://newflow.ee/nodes/MyNode?utm_source=n8n_app&utm_medium=node_settings_modal-credential_link&utm_campaign=n8n-nodes-base.set',
		);
	});
	...
	it('returns builtin docs root with UTM if no other match', () => {
		...
		expect(docsUrl.value).toEqual(
			'http://newflow.ee/integrations/builtin/?utm_source=n8n_app&utm_medium=node_settings_modal-credential_link&utm_campaign=n8n-nodes-base.set',
		);
	});
