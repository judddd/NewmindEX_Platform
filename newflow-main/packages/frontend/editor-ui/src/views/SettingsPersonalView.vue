...
import { useRootStore } from '@newflow/stores';
import { createFormEventBus } from '@newflow/design-system/utils';
...
async function updatePersonalisationSettings() {
	...
	// NewFlow: Save language preference and load language pack
	if (currentSelectedLanguage.value !== rootStore.defaultLocale) {
		try {
			// Dynamically load the language pack if not English
			if (currentSelectedLanguage.value === 'zh') {
				const { loadLanguage } = await import('@newflow/i18n');
				const zhMessages = await import('@newflow/i18n/locales/zh.json');
				loadLanguage('zh', zhMessages.default);
			} else {
				// For English, just set the language
				const { setLanguage } = await import('@newflow/i18n');
				setLanguage(currentSelectedLanguage.value);
			}

			rootStore.setDefaultLocale(currentSelectedLanguage.value);
			await usersStore.updateUserSettings({ language: currentSelectedLanguage.value });
		} catch (error) {
			console.error('Failed to load language pack:', error);
			showError(error as Error, i18n.baseText('settings.personal.personalSettingsUpdatedError'));
		}
	}
}
...
<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		...
		<div v-if="isPersonalSecurityEnabled">
			...
			<!-- NewFlow: 隐藏 2FA 设置（appliance 模式不需要多因素认证） -->
			<!--
