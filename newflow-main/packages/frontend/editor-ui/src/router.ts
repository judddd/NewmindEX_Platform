...
import { MfaRequiredError } from '@newflow/rest-api-client';
...
const AIAgentsView = async () => await import('@/views/AIAgentsView.vue');
...
export const routes: RouteRecordRaw[] = [
	...
	// NewFlow: Variables route removed
	// {
	// 	path: '/variables',
	...
	// NewFlow: Evaluation route removed
	// {
	...
	},
	{
		...
		meta: {
			// NewFlow Cloud: Allow access for both owner setup and invitation code registration
			middleware: ['guest'],
			telemetry: {
				pageCategory: 'auth',
			},
	...
	},
	{
		...
		props: true,
		// Always redirect to Personal Settings (Usage page removed)
		redirect: { name: VIEWS.PERSONAL_SETTINGS },
		children: [
			// Usage page hidden - redirect handled at parent level
			{
				path: 'usage',
				name: VIEWS.USAGE,
				redirect: { name: VIEWS.PERSONAL_SETTINGS },
			...
			// NewFlow: Source Control route removed
			// {
			// 	path: 'environments',
			...
			// 	},
			// },
			// NewFlow: External Secrets route removed
			// {
			...
			// 	},
			// },
			// NewFlow: SSO settings route removed
			// {
			// 	path: 'sso',
			...
			// 	},
			// },
			// NewFlow: Log Streaming route removed
			// {
			// 	path: 'log-streaming',
			...
			// NewFlow: LDAP settings route removed
			// {
			// 	path: 'ldap',
	...
	// NewFlow: SAML onboarding route removed
	// {
	...
	{
		path: '/ai-agents',
		name: VIEWS.AI_AGENTS,
		components: {
			default: AIAgentsView,
			sidebar: MainSidebar,
		},
		meta: {
			middleware: ['authenticated'],
			telemetry: {
				pageCategory: 'ai-agents',
			},
		},
	},
	...projectsRoutes,
...
router.beforeEach(async (to: RouteLocationNormalized, from, next) => {
	try {
		...
		const settingsStore = useSettingsStore();
		// NewFlow: Skip setup page if auto-setup is enabled (backend handles it)
		if (settingsStore.showSetupPage && !settingsStore.isAutoSetupMode) {
