...
export async function initializeCore() {
	...
	// NewFlow: Simplified banner logic - no license warnings
	const banners: BannerName[] = [];
	// showNonProdBanner check removed - never show license warnings
...
export async function initializeAuthenticatedFeatures(
	...
	// NewFlow: Load language from user settings
	if (usersStore.currentUser?.settings?.language) {
		const userLanguage = usersStore.currentUser.settings.language;
		rootStore.setDefaultLocale(userLanguage);
