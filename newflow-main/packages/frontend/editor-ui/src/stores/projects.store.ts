...
import * as workflowsEEApi from '@/api/workflows';
import * as credentialsApi from '@/api/credentials';
import * as credentialsEEApi from '@/api/credentials';
...
import { getResourcePermissions } from '@newflow/permissions';
import type { CreateProjectDto, UpdateProjectDto } from '@newflow/api-types';
...
export const useProjectsStore = defineStore(STORES.PROJECTS, () => {
	...
	const teamProjectsLimit = computed(() => 0); // NewFlow: Team projects disabled
	const isTeamProjectFeatureEnabled = computed<boolean>(() => false); // NewFlow: Always disabled
	const hasUnlimitedProjects = computed<boolean>(() => false); // NewFlow: Team projects disabled
	const isTeamProjectLimitExceeded = computed<boolean>(
		() => projectsCount.value.team >= teamProjectsLimit.value,
	);
	// NewFlow: Team projects disabled - cannot create new projects
	const canCreateProjects = computed<boolean>(() => false);
