...
describe('BinaryDataConfig', () => {
	...
	it('should use values from env variables when defined', () => {
		process.env.NEWFLOW_AVAILABLE_BINARY_DATA_MODES = 'filesystem,s3';
		process.env.NEWFLOW_DEFAULT_BINARY_DATA_MODE = 's3';
		process.env.NEWFLOW_BINARY_DATA_STORAGE_PATH = '/custom/storage/path';
		process.env.NEWFLOW_BINARY_DATA_SIGNING_SECRET = 'super-secret';
	...
	it('should fallback to default for mode', () => {
		process.env.NEWFLOW_DEFAULT_BINARY_DATA_MODE = 'invalid-mode';
	...
	it('should fallback to default for available modes', () => {
		process.env.NEWFLOW_AVAILABLE_BINARY_DATA_MODES = 'filesystem,invalid-mode,s3';
