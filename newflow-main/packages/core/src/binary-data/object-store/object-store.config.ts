...
class ObjectStoreBucketConfig {
	...
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_BUCKET_NAME')
	name: string = '';

	/** Region of the n8n bucket in S3-compatible external storage @example "us-east-1" */
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_BUCKET_REGION')
	region: string = '';
}
...
class ObjectStoreCredentialsConfig {
	...
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_ACCESS_KEY')
	accessKey: string = '';

	/** Access secret in S3-compatible external storage */
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_ACCESS_SECRET')
	accessSecret: string = '';
	 ...
	 */
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_AUTH_AUTO_DETECT')
...
export class ObjectStoreConfig {
	 ...
	 **/
	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_HOST')
	host: string = '';

	@Env('NEWFLOW_EXTERNAL_STORAGE_S3_PROTOCOL', protocolSchema)
