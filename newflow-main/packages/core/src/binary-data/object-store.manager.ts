...
export class ObjectStoreManager implements BinaryData.Manager {
	...
	async getAsStream(fileId: string): Promise<Readable> {
	...
	async copyByFileId(workflowId: string, executionId: string, sourceFileId: string) {
		...
		const sourceFile: Buffer = await this.objectStoreService.get(sourceFileId, { mode: 'buffer' });
	...
	async rename(oldFileId: string, newFileId: string) {
		const oldFile: Buffer = await this.objectStoreService.get(oldFileId, { mode: 'buffer' });
