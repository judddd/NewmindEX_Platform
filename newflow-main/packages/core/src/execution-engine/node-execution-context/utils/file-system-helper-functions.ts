...
import { isContainedWithin, safeJoinPath } from '@newflow/backend-common';
...
import {
	...
	BLOCK_FILE_ACCESS_TO_NEWFLOW_FILES,
	CONFIG_FILES,
...
export async function isFilePathBlocked(filePath: string): Promise<boolean> {
	...
	const blockFileAccessToNewflowFiles = process.env[BLOCK_FILE_ACCESS_TO_NEWFLOW_FILES] !== 'false';

	const restrictedPaths = blockFileAccessToNewflowFiles ? getN8nRestrictedPaths() : [];
	if (
