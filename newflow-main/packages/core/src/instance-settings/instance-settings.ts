...
import { InstanceSettingsConfig } from '@newflow/config';
...
export class InstanceSettings {
	...
	private loadOrCreate(): Settings {
		const encryptionKeyFromEnv = process.env.NEWFLOW_ENCRYPTION_KEY;
		if (existsSync(this.settingsFile)) {
			...
			if (encryptionKeyFromEnv && encryptionKey !== encryptionKeyFromEnv) {
				throw new ApplicationError(
					`Mismatching encryption keys. The encryption key in the settings file ${this.settingsFile} does not match the NEWFLOW_ENCRYPTION_KEY env var. Please make sure both keys match. More information: http://newflow.ee/hosting/environment-variables/configuration-methods/#encryption-key`,
				);
			}
	...
	private getOrGenerateHmacSignatureSecret() {
		const hmacSignatureSecretFromEnv = process.env.NEWFLOW_HMAC_SIGNATURE_SECRET;
	...
	private loadEnforceSettingsFilePermissionsFlag(): {
		...
		const isEnvVarSet = !!process.env.NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS;
		if (this.isWindows()) {
			if (isEnvVarSet) {
				console.warn(
					'Ignoring NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS as it is not supported on Windows.',
				);
			}
	...
	private ensureSettingsFilePermissions() {
		...
		if (!permissionsResult.ok) {
			this.logger.warn(
				`Could not ensure settings file permissions: ${permissionsResult.error.message}. To skip this check, set NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
			);
			return;
		}
		...
		if (!this.enforceSettingsFilePermissions.isSet) {
			this.logger.warn(
				`Permissions 0${permissionsResult.result.toString(8)} for n8n settings file ${this.settingsFile} are too wide. This is ignored for now, but in the future n8n will attempt to change the permissions automatically. To automatically enforce correct permissions now set NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS=true (recommended), or turn this check off set NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
			);
		...
		if (this.enforceSettingsFilePermissions.enforce) {
			...
			if (!chmodResult.ok) {
				...
				// future in this case.
				this.logger.warn(
					`Could not enforce settings file permissions: ${chmodResult.error.message}. To skip this check, set NEWFLOW_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
				);
			}
		}
	}
