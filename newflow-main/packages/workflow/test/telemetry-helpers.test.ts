...
describe('generateNodesGraph', () => {
		...
		const workflow: IWorkflowBase = {
			...
			nodes: [
				...
				},
				{
					parameters: {
						content:
							"test\n\n## I'm a note \n**Double click** to edit me. [Guide](http://newflow.ee/workflows/sticky-notes/)",
					},
	...
	test('should return node graph with agent node and all prompt types when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					id: 'agent-node-id',
					name: 'Agent Node',
					type: '@newflow/n8n-nodes-langchain.agent',
					typeVersion: 1,
					position: [100, 100],
				},
				{
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.agent', 'n8n-nodes-base.set'],
				node_connections: [{ start: '0', end: '1' }],
				nodes: {
					'0': {
						id: 'agent-node-id',
	...
	test('should return node graph with agent tool node and prompt text when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					type: '@newflow/n8n-nodes-langchain.agentTool',
					typeVersion: 1,
					position: [300, 300],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.agentTool'],
				node_connections: [],
				nodes: {
					'0': {
						id: 'agent-tool-node-id',
	...
	test('should return node graph with openai langchain node and prompts array when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					id: 'openai-node-id',
					name: 'OpenAI Node',
					type: '@newflow/n8n-nodes-langchain.lmChatOpenAi',
					typeVersion: 1,
					position: [400, 400],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.lmChatOpenAi'],
				node_connections: [],
				nodes: {
					'0': {
						id: 'openai-node-id',
	...
	test('should return node graph with chain summarization node and summarization prompts when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					type: '@newflow/n8n-nodes-langchain.chainSummarization',
					typeVersion: 1,
					position: [500, 500],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.chainSummarization'],
				node_connections: [],
				nodes: {
					'0': {
						id: 'summarization-node-id',
	...
	test('should return node graph with langchain custom tool node and description prompt when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					type: '@newflow/n8n-nodes-langchain.customTool',
					typeVersion: 1,
					position: [600, 600],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.customTool'],
				node_connections: [],
				nodes: {
					'0': {
						id: 'custom-tool-node-id',
	...
	test('should return node graph with chain llm node and messageValues prompts when cloud telemetry is enabled', () => {
			...
			nodes: [
				{
					...
					name: 'Chain LLM Node',
					type: '@newflow/n8n-nodes-langchain.chainLlm',
					typeVersion: 1,
					position: [700, 700],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes, { isCloudDeployment: true })).toEqual({
			nodeGraph: {
				node_types: ['@newflow/n8n-nodes-langchain.chainLlm'],
				node_connections: [],
				nodes: {
					'0': {
						id: 'chain-llm-node-id',
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: [
					...
					'@newflow/n8n-nodes-langchain.chainLlm',
			...
			nodes: [
				{
					...
					id: 'chat-trigger-id',
					name: 'Chat Trigger',
					type: '@newflow/n8n-nodes-langchain.chatTrigger',
					typeVersion: 1,
					position: [100, 100],
				},
				{
