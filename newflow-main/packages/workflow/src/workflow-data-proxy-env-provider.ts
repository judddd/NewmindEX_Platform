...
export function createEnvProviderState(): EnvProviderState {
	...
	const isEnvAccessBlocked = isProcessAvailable
		? process.env.NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE === 'true'
		: false;
			...
			get(_, name) {
				...
				if (providerState.isEnvAccessBlocked) {
					throw new ExpressionError('access to env vars denied', {
						causeDetailed:
							"If you need access please contact the administrator to remove the environment variable 'NEWFLOW_BLOCK_ENV_ACCESS_IN_NODE'",
						runIndex,
						itemIndex,
					});
				}
